...
GEMINI_MODEL_LABELS = {
    "gemini-3.7-flash": "🔥 gemini-3.7-flash — ល្អបំផុត",
    "gemini-3.6-flash": "⚡ gemini-3.6-flash — លឿន និងមានតុល្យភាព",
    "gemini-3.5-flash": "🚀 gemini-3.5-flash — ស្តង់ដារ",
    "gemini-3.5-flash-lite": "💡 gemini-3.5-flash-lite — សន្សំ និងលឿន",
...
def save_upload(uploaded_file):
    ...
    if suffix not in allowed_suffixes:
        raise ValueError("ទម្រង់វីដេអូមិនត្រូវគាំទ្រទេ។ សូមប្រើ MP4, MOV, MKV ឬ WEBM។")
    ...
    try:
        ...
        if not destination.exists() or destination.stat().st_size == 0:
            raise RuntimeError("ឯកសារវីដេអូទទេ ឬអាប់ឡូតមិនពេញលេញ។")
        return destination
    except Exception as exc:
        destination.unlink(missing_ok=True)
        raise RuntimeError(f"មិនអាចរក្សាទុកវីដេអូដែលបានអាប់ឡូត៖ {exc}") from exc
...
def validate_video_duration(video_path):
    ...
    if seconds > VIDEO_MAX_DURATION_SECONDS:
        raise ValueError("វីដេអូត្រូវមានរយៈពេល 10 នាទីចុះក្រោម។ សូមកាត់វីដេអូឱ្យខ្លីសិន។")
    return seconds
...
def save_background_music_upload(uploaded_file):
    ...
    if suffix not in allowed_suffixes:
        raise ValueError('ភ្លេងត្រូវជា MP3, WAV, M4A, AAC ឬ OGG។')
    size = int(getattr(uploaded_file, 'size', 0) or 0)
    if size <= 0 or size > BACKGROUND_MUSIC_MAX_MB * 1024 * 1024:
        raise ValueError(f'ភ្លេងត្រូវមានទំហំតិចជាង {BACKGROUND_MUSIC_MAX_MB} MB។')
    workspace = _ensure_project_workspace()
    ...
    try:
        ...
        if not destination.exists() or destination.stat().st_size < 256:
            raise RuntimeError('ឯកសារភ្លេងទទេ ឬអាប់ឡូតមិនពេញលេញ។')
        return destination
    except Exception as exc:
        destination.unlink(missing_ok=True)
        raise RuntimeError(f'មិនអាចរក្សាទុកភ្លេងដែលបានអាប់ឡូត៖ {exc}') from exc
...
def render_audio_ducking_controls(context_key):
    ...
    with st.expander('🎵 ភ្លេងកំដរ និង Auto Ducking', expanded=False):
        st.caption('ភ្លេងនឹងថយស្រាលៗពេលមានសំឡេងនិយាយ ហើយត្រឡប់ឡើងវិញដោយរលូន។')
        ...
        if music_path and music_path.exists():
            st.caption(f'✅ បានជ្រើសភ្លេង៖ {music_path.name}')
        else:
            st.caption('បើមិនជ្រើសភ្លេង App នឹងបង្កើត MP3 សំឡេងធម្មតា ដូចមុន។')

        enabled = st.toggle(
            '🎚️ បើក Auto Ducking', value=stored['enabled'],
            disabled=not (music_path and music_path.exists()), key=f'ducking_enabled_{context_key}',
        )
        c1, c2 = st.columns(2, gap='small')
        with c1:
            music_gain = st.slider('កម្រិតភ្លេង', 0.10, 0.70, stored['music_gain'], 0.01,
                                   key=f'ducking_gain_{context_key}')
            ratio = st.slider('កម្លាំង Ducking', 2.0, 12.0, stored['ratio'], 0.5,
                              key=f'ducking_ratio_{context_key}')
        with c2:
            attack_ms = st.slider('ចុះភ្លេង', 15, 180, stored['attack_ms'], 5,
                                  key=f'ducking_attack_{context_key}')
            release_ms = st.slider('ឡើងភ្លេងវិញ', 250, 1600, stored['release_ms'], 50,
...
def extract_audio(video_path, audio_path, fast_mode=True):
    ...
    try:
        ...
        raise RuntimeError("ការទាញសំឡេងចំណាយពេលយូរពេក។ សូមសាកវីដេអូខ្លីជាង ឬបង្រួមវីដេអូ។") from exc
    except FileNotFoundError as exc:
        raise RuntimeError("ម៉ាស៊ីនមេមិនមាន FFmpeg សម្រាប់ដំណើរការវីដេអូទេ។") from exc
...
def upload_for_context(client, video_path):
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise RuntimeError("Gemini ចំណាយពេលយូរពេកក្នុងការរៀបចំវីដេអូ។ សូមសាកម្ដងទៀត ឬកែ SRT ដោយគ្មានវីដេអូ។")
        time.sleep(2)
        uploaded = client.files.get(name=uploaded.name)
    if state in {"FAILED", "ERROR"}:
        raise RuntimeError("Gemini មិនអាចអានវីដេអូនេះបានទេ។")
    if state == "PROCESSING":
        raise RuntimeError("Gemini មិនទាន់ត្រៀមវីដេអូរួចរាល់ទេ។")
    return uploaded
...
def is_structured_output_error(exc):
    message = _error_message(exc)
    return "JSON" in message or "AI មិនបានត្រឡប់បន្ទាត់" in str(exc or "")
...
def build_srt(cues, translated):
    ...
    for cue in cues:
        ...
        dialogue = normalize_dialogue(item.get("text", ""))
        if not dialogue or contains_non_khmer_script(dialogue):
            raise RuntimeError(f"បន្ទាត់ {cue['id']} មិនមែនជាអក្សរខ្មែរសុទ្ធទេ។")
...
def friendly_ai_error(exc, key_count=1):
    ...
    return f"AI មិនអាចបញ្ចប់ការបកប្រែបាន៖ {message[:280]}"
...
def translate_srt_to_khmer(srt_text, api_keys, model, source_language="Auto-detect", translation_style="👤 បកប្រែធម្មតា (Standard)", target_language="Khmer (ខ្មែរ)"):
    ...
    if not source_cues:
        raise ValueError("SRT មិនត្រឹមត្រូវ ឬមិនមានអត្ថបទសម្រាប់បកប្រែទេ។")
    cues = [
        {
            "id": cue["id"],
...
def create_mp3(srt_text, progress_callback=None, background_music_path=None, ducking_config=None, target_language="Khmer (ខ្មែរ)"):
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        if progress_callback:
            progress_callback(
                92,
                "កំពុងលាយសំឡេង និងបន្ថយភ្លេងដោយស្វ័យប្រវត្តិ…"
                if use_ducking else "កំពុងបញ្ចូលសំឡេងទាំងអស់ជាបទ MP3 តែមួយ…",
            )
...
def create_single_voice_mp3(text, tag, background_music_path=None, ducking_config=None, target_language="Khmer (ខ្មែរ)"):
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        if result.returncode != 0 or not output.exists() or output.stat().st_size < 1000:
            raise RuntimeError(result.stderr[-1600:] or 'មិនអាចសម្រួលសំឡេង MP3 បានទេ។')
        return output.read_bytes()


def render_thought_voice_guide():
    """Show a compact, practical guide wherever customers choose a voice tag."""
    with st.expander("📘 របៀបប្រើ [M_THINK] និង [F_THINK]", expanded=False):
        st.markdown(
            "**[M_THINK]** សម្រាប់គំនិតក្នុងចិត្តរបស់តួប្រុស និង "
            "**[F_THINK]** សម្រាប់គំនិតក្នុងចិត្តរបស់តួស្រី។ "
            "ប្រើតែពេលតួអង្គកំពុងគិតដោយស្ងាត់ៗ ឬអ្នកដទៃក្នុងឆាកមិនអាចឮ។"
        )
        st.markdown(
            "ដាក់ tag នៅដើមបន្ទាត់ និងសរសេរឃ្លាខ្លីរលូន។ កុំដាក់ `_THINK` "
            "លើសន្ទនាដែលតួអង្គនិយាយចេញមកក្រៅ។ សំឡេងគិតក្នុងចិត្តត្រូវទន់ជាង "
            "ប៉ុន្តែមិនមាន echo និងមិនធ្វើឱ្យស្តាប់ដូចនៅក្នុងពាងទេ។"
        )
        st.code(
            """3
...
[F_THINK] សង្ឃឹមថាគាត់មិនលាក់រឿងសំខាន់ពីខ្ញុំទេ។""",
        ...
        st.caption("គន្លឹះ៖ រក្សា cue ឱ្យមានពេលគ្រប់គ្រាន់ និងកុំសរសេរឃ្លាវែងពេក ដើម្បីឱ្យចង្វាក់ TTS ធម្មជាតិ។")
...
def restore_license_backup(backup_bytes):
    ...
    try:
        ...
        raise ValueError("Backup JSON មិនត្រឹមត្រូវ។") from exc
    if not isinstance(payload, dict) or int(payload.get("schema_version", 0)) != LICENSE_BACKUP_SCHEMA_VERSION:
        raise ValueError("Backup version មិនត្រឹមត្រូវ។")
    records = payload.get("licenses")
    if not isinstance(records, list) or len(records) > 10000:
        raise ValueError("Backup មិនមានបញ្ជី Access Code ត្រឹមត្រូវ។")
    ...
    with license_connection() as connection:
        for record in records:
            ...
            try:
                ...
                plan = str(record.get("plan_label") or "កញ្ចប់សមាជិក").strip()[:80]
...
def render_private_subscription_countdown(expiry_datetime, plan_label, customer_name="", access_code=""):
    ...
    safe_name = re.sub(r"[^0-9A-Za-z\u1780-\u17FF .\-]", "", str(customer_name or "សមាជិក"))
    safe_code = re.sub(r"[^0-9A-Za-z\-]", "", str(access_code or ""))

    components.html(
        f"""
        <div id="khbr-card" class="khbr-countdown-card" aria-label="ព័ត៌មានកញ្ចប់របស់អ្នក">
          <div class="khbr-card-top"><span class="khbr-live-dot"></span><span>គណនីរបស់អ្នក</span></div>
          <div class="khbr-name">👋 {safe_name or "សមាជិក"}</div>
          <div class="khbr-meta"><span>🏷️ {safe_plan or "កញ្ចប់សមាជិក"}</span>{f'<span>• លេខកូដ {safe_code}</span>' if safe_code else ''}</div>
          <div class="khbr-expiry-row"><span class="khbr-calendar">🗓️</span><span>ផុតកំណត់</span><strong id="khbr-expiry">កំពុងគណនា…</strong></div>
          <div class="khbr-remaining"><strong id="khbr-days">—</strong><span>ថ្ងៃនៅសល់</span></div>
          <div id="khbr-detail" class="khbr-detail">កំពុងគណនា…</div>
        </div>
        <style>
        ...
        <script>
          ...
          function updateCountdown() {{
            ...
            if (ms <= 0) {{
              ...
              detailNode.textContent = "កញ្ចប់បានផុតកំណត់";
              return;
            }}
            ...
            detailNode.textContent = "នៅសល់ " + two(hours) + " ម៉ោង " + two(minutes) + " នាទី";
          }}
...
def admin_dashboard():
    ...
    st.markdown("## 🛡️ សុវត្ថិភាព API & Access Code")
    with st.expander("🔐 គ្រប់គ្រង API Key និង Backup Access Code", expanded=False):
        server_keys = [line for line in load_secret_gemini_api_keys().splitlines() if line]
        if server_keys:
            st.success(f"✅ App API Key ពី Streamlit Secrets ត្រៀមប្រើ៖ {len(server_keys)}")
        else:
            st.warning("⚠️ មិនទាន់មាន App API Key ក្នុង Streamlit Secrets។")
        st.caption(
            "តម្លៃ API Key មិនត្រូវបង្ហាញ ឬរក្សាទុកក្នុង Owner dashboard ទេ។ "
            "ដើម្បីប្តូរ App API Key សូមកែ GEMINI_API_KEYS ក្នុង Streamlit Secrets ដោយផ្ទាល់។"
        )
        st.divider()
        st.markdown("#### 💾 Backup Access Code")
        st.caption(
            "ទាញយក Backup មុន update ឬ reboot។ ហ្វាលនេះមានឈ្មោះ, Access Code, Plan និងថ្ងៃផុតកំណត់ "
            "ប៉ុន្តែមិនមាន API Key ទេ។ សូមរក្សាទុកជាឯកជន និងកុំចែករំលែក។"
        )
        st.download_button(
            "⬇️ ទាញយក Backup Access Code", export_license_backup(),
        ...
        restore_file = st.file_uploader(
            ...
            help="Restore នាំចូលតែកូដដែលមិនទាន់មាន។ កូដដែលមានរួចមិនត្រូវ overwrite ឬបង្កើតថ្មីទេ។",
        )
        if st.button("↥ Restore កូដដែលបាត់", key="restore_license_backup", disabled=restore_file is None, use_container_width=True):
            try:
                result = restore_license_backup(restore_file.getvalue())
                st.success(f"✅ បាននាំចូល {result['imported']} កូដ • រំលង {result['skipped']} កូដដែលមានរួច/មិនត្រឹមត្រូវ")
            except Exception as exc:
                st.error(f"❌ Restore មិនបាន៖ {exc}")

    st.markdown("## ➕ បង្កើត Customer")
    st.caption("Owner បង្កើត Access Code ម្តងមួយ សម្រាប់ Customer ម្នាក់។ Code មួយអាច Login លើ iPhone, Android និង Browser ផ្សេងៗបាន ដោយមិនចងជាមួយឧបករណ៍។")
        ...
        with st.expander(f"{row['customer_name']} • {row['access_code_display']} • {status}"):
            ...
            with action_middle:
                if st.button("សម្អាត Session ចាស់", key=f"disconnect_{row['id']}", use_container_width=True):
                    ...
                    st.rerun()
            with action_right:
                if st.button("🗑️ លុប API Key ចាស់ក្នុង Database", key=f"owner_delete_api_{row['id']}", use_container_width=True):
                    with license_connection() as connection:
                        connection.execute(
                    ...
                    st.success("Owner បានលុប API Key ចាស់ក្នុង Database រួច។ API Key ផ្ទាល់ខ្លួននៅក្នុង browser របស់ Customer មិនត្រូវបានបង្ហាញ ឬប៉ះពាល់ទេ។")
                    st.rerun()
...
for state_key, default_value in {
    ...
    "source_language": "Auto-detect (ចិន/កូរ៉េ/វៀតណាម/អង់គ្លេស)",
    "workflow_mode": "⚡ Khmer SRT ស្វ័យប្រវត្តិ",
    "processing_mode": "⚡ លឿន (ណែនាំ)",
    "translation_style": "👤 បកប្រែធម្មតា (Standard)",
...
with st.container(key="api_menu_container"):
    with st.popover("☰", help="API Key និងការកំណត់កម្មវិធី"):
        ...
        st.selectbox(
            ...
            help="Standard សម្រាប់សន្ទនាធម្មជាតិ; Lipsync សម្រាប់ឃ្លាខ្លីស៊ី timing; Comedy សម្រាប់កំប្លែង; Formal សម្រាប់ភាសាគួរសម។",
        )

        with st.expander("⚙️ ជម្រើសលំហូរបន្ថែម", expanded=False):
            st.selectbox(
                "🗣️ ភាសាវីដេអូ/អក្សរដើម",
                [
            ...
            st.selectbox(
                ...
                key="workflow_mode",
                help="ជ្រើសតែម្ដង ហើយកម្មវិធីនឹងដំណើរការតាមលំហូរដែលអ្នកជ្រើស។",
            )
            st.selectbox(
                "⚙️ ល្បឿនដំណើរការ",
        ...
        if st.button("💾 រក្សាទុក Settings & API Key", key="save_api_keys", use_container_width=True):
            entered_keys = [
                line.strip()
            ...
            if preferences_saved and keys_saved:
                ...
                st.error("មិនអាចរក្សាទុក Settings បានទេ។ API Key ចាស់ និង Settings ចាស់នៅតែរក្សាទុកដដែល។ សូមសាកល្បងម្តងទៀត។")
...
with tab_video:
    ...
    with st.container(key="main_video_upload_panel"):
        st.markdown(
            '<div class="upload-note">📹 <strong>អាប់ឡូតវីដេអូ</strong> • MP4/MOV/MKV/WEBM • អតិបរមា 10 នាទី • 100 MB</div>',
            unsafe_allow_html=True,
        )
        uploaded_video = st.file_uploader(
            "ជ្រើសរើសវីដេអូ (10 នាទីចុះក្រោម)",
            type=["mp4", "mov", "mkv", "webm"],
            help="សម្រាប់ upload លឿន សូមប្រើ MP4 720p ឬ 480p ដែលតូចជាង 100 MB។ App ទាញតែសំឡេង mono 16 kHz សម្រាប់ ASR ដើម្បីប្រើ memory តិច។",
    ...
    if uploaded_video is not None:
        ...
        if size_mb > max_mb:
            ...
            st.caption(f"លំហូរដែលបានជ្រើស៖ {workflow_mode} • ប្រភព៖ {source_language} • របៀប៖ {'លឿន' if fast_mode else 'សំឡេងច្បាស់'}")
            if st.button("🚀 ចាប់ផ្ដើមលំហូរដែលបានជ្រើស", key="generate_srt", use_container_width=True):
                video_path = None
                ...
                try:
                    ...
                    show_waiting(
                        "កំពុងរៀបចំសំឡេងពីវីដេអូ",
                        f"រយៈពេល {int(duration_seconds // 60)} នាទី {int(duration_seconds % 60)} វិនាទី • សូមរង់ចាំបន្តិច និងកុំបិទទំព័រនេះ។",
                    )
                    ...
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        ...
                        while not future.done():
                            ...
                            show_waiting("កំពុងស្គាល់សំឡេង", "AI កំពុងទាញសន្ទនាពីវីដេអូរបស់អ្នក…")
                            time.sleep(0.35)
                ...
                if not st.session_state.srt_text.strip():
                    ...
                    try:
                        ...
                        waiting.markdown(
                            ...
                            '<div><div class="khemra-wait-title">កំពុងកែ SRT ឱ្យរលូន</div>'
                            '<div class="khemra-wait-copy">កំពុងរក្សាតួអង្គ កែស្លាក និងចង្វាក់សន្ទនា…</div>'
...
st.caption("AI-KHEMRA-BRO v6.7.2 • Clear 40% Thought Voice • Mobile-first")
